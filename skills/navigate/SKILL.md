...
3. **Identify an index gap** - If the user confirms their request was valid
   ...
   - Offer next steps:
     - `/hiivmind-corpus:hiivmind-corpus enhance flyio {topic}` - Add depth to a specific topic area
     - `/hiivmind-corpus:hiivmind-corpus add-source flyio` - Add another documentation source
     - `/hiivmind-corpus:hiivmind-corpus refresh flyio` - Check if upstream docs have new content
...
```
/hiivmind-corpus:hiivmind-corpus refresh flyio     - Update index from upstream
/hiivmind-corpus:hiivmind-corpus enhance flyio X   - Add depth to topic X
/hiivmind-corpus:hiivmind-corpus status flyio      - Check corpus freshness
```
